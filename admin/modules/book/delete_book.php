...
if (is_submit('delete_book'))
{
    // Lấy ID và ép kiểu
    $id = (int)input_post('book_id');
    if ($id)
    {
        // Lấy thông tin người dùng
        $user = db_get_row(db_create_sql('SELECT * FROM bang_sach {where}', array(
            'id' => $id
        )));
        ...
        {
            $sql = db_create_sql('DELETE FROM bang_sach {where}', array(
                'id' => $id
            ));
